...
class Pages:
    ...
    def uploadImage(self, pagename, source, width, height, msPerFrame, name,
        ...
        elif len(uploadedImage) < 4  or  uploadedImage[1:4] != "PNG":
            errormessage = "Images must be in PNG format."
        elif len(uploadedImage) > 102400:
            errormessage = "Image files must be smaller than 100K."
        else:
            fname = cherrypy.request.filenameMap['uploadedImage'].strip()
